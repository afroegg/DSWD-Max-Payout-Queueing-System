...
/* DUPLICATE CHECK:
   same first_name + last_name + contact_number
   with active queue today */
$dup = $conn->prepare("
...
if ($dup_result->num_rows > 0) {
    ...
    echo "<script>
        alert('Duplicate queue detected. This beneficiary already has an active queue today.\\nQueue No: {$existingQueue}\\nStatus: {$existingStatus}');
        window.history.back();
    </script>";
    exit;
}
...
/* GENERATE SHORT QUEUE NUMBER: MPO-00001 */
...
echo "<script>
    alert('Registration successful. Queue Number: {$queue_number}');
