...
try{
    ...
    $allowedPlaces=['Oriental Mindoro'=>['Calapan City','Baco','Bansud','Bongabong','Bulalacao','Gloria','Mansalay','Naujan','Pinamalayan','Pola','Puerto Galera','Roxas','San Teodoro','Socorro','Victoria'],'Occidental Mindoro'=>['Abra de Ilog','Calintaan','Looc','Lubang','Magsaysay','Mamburao','Paluan','Rizal','Sablayan','San Jose','Santa Cruz'],'Marinduque'=>['Boac','Buenavista','Gasan','Mogpog','Santa Cruz','Torrijos'],'Romblon'=>['Alcantara','Banton','Cajidiocan','Calatrava','Concepcion','Corcuera','Ferrol','Looc','Magdiwang','Odiongan','Romblon','San Agustin','San Andres','San Fernando','San Jose','Santa Fe'],'Palawan'=>['Aborlan','Agutaya','Araceli','Balabac','Bataraza','Brooke’s Point','Busuanga','Cagayancillo','Coron','Culion','Cuyo','Dumaran','El Nido','Kalayaan','Linapacan','Magsaysay','Narra','Quezon','Rizal','Roxas','San Vicente','Sofronio Española','Taytay'],'Puerto Princesa City'=>['Puerto Princesa City']];
    ...
    if(!isset($allowedPlaces[$province])) reply_json(false,['message'=>'MIMAROPA address only.']);
    if(!in_array($city,$allowedPlaces[$province],true)) reply_json(false,['message'=>'City/Municipality must match selected province.']);
    if($barangay===''||$street==='') reply_json(false,['message'=>'Complete address is required.']);
...
}catch(Throwable $e){ if(isset($conn)&&$conn->errno===0){@ $conn->rollback();} reply_json(false,['message'=>'Queue registration failed. '.$e->getMessage()]); }
?>
