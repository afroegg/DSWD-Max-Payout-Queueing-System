...
$first = val($input, 'first_name');
$middle = val($input, 'middle_name');
$last = val($input, 'last_name');
$ext = val($input, 'ext_name');
$phone = phone_clean($input['contact_number'] ?? '');
$province = val($input, 'province');
$city = val($input, 'city_municipality');
$barangay = val($input, 'barangay');
$street = val($input, 'street_address');
$program = val($input, 'program_type');

$provinces = ['Oriental Mindoro','Occidental Mindoro','Marinduque','Romblon','Palawan','Puerto Princesa City'];
...
function nextCode($conn) {
    ...
    $n = 1;
    if ($res && $res->num_rows > 0) { $r = $res->fetch_assoc(); if ($r['last_number'] !== null) $n = intval($r['last_number']) + 1; }
    return 'PAL-' . str_pad($n, 5, '0', STR_PAD_LEFT);
}
function nextQueue($conn) {
    $res = $conn->query("SELECT MAX(CAST(SUBSTRING(queue_number, 5) AS UNSIGNED)) AS last_number FROM queue_entries WHERE DATE(transaction_date)=CURDATE() AND queue_type='regular' AND queue_number LIKE 'PAL-%'");
    $n = 1;
    if ($res && $res->num_rows > 0) { $r = $res->fetch_assoc(); if ($r['last_number'] !== null) $n = intval($r['last_number']) + 1; }
    return 'PAL-' . str_pad($n, 4, '0', STR_PAD_LEFT);
}

$code = nextCode($conn);
$region = 'MIMAROPA';
$birth_m = 1; $birth_d = 1; $birth_y = 2000; $age = 0; $sex = 'Male';
$lgu = $city; $id = 'For Verification'; $hh = ''; $sms = 0; $preg = 0;
...
try {
    ...
    $stmt->bind_param('ssssssiiiisssssssssii', $code, $first, $middle, $last, $ext, $phone, $birth_m, $birth_d, $birth_y, $age, $sex, $lgu, $id, $hh, $program, $region, $province, $city, $barangay, $sms, $preg);
    if (!$stmt->execute()) throw new Exception('Unable to save record.');
    $beneficiary_id = $conn->insert_id;
    $queue_no = nextQueue($conn);
    $qtype = 'regular';
    ...
    $conn->commit();
    reply_json(true, ['queue_number'=>$queue_no,'queue_type'=>$qtype,'program_type'=>$program,'beneficiary_code'=>$code,'client_name'=>trim($first.' '.$middle.' '.$last.' '.$ext),'contact_number'=>$phone,'address'=>trim($street.', '.$barangay.', '.$city.', '.$province.', MIMAROPA'),'region'=>'MIMAROPA','date_time'=>date('M d, Y h:i A')]);
} catch (Throwable $e) {
    $conn->rollback();
    reply_json(false, ['message'=>$e->getMessage()]);
}
?>
