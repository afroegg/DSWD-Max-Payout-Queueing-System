...
while ($row = $result->fetch_assoc()) {
    $is_pwd = intval($row['sms_opt_in'] ?? 0);
    $is_pregnant = intval($row['is_pregnant'] ?? 0);
    $age = intval($row['age'] ?? 0);

    if ($is_pwd === 1) {
        $category = 'PWD';
    } elseif ($is_pregnant === 1) {
        $category = 'Pregnant';
    } elseif ($age >= 60) {
        $category = 'Senior';
    } else {
