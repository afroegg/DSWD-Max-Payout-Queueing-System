...
if ($row['workflow_status'] !== 'CALLED_STEP_3') goBack($counter_number, 'Only called Step 3 queues can be marked as paid.');
...
if (!$formResult || $formResult->num_rows === 0) {
    goBack($counter_number, 'GIS form must be approved and locked before marking paid.');
}
...
if ($update->execute()) goBack($counter_number, 'Queue marked as paid: ' . $row['queue_number']);
