...
if (!$formResult || $formResult->num_rows === 0) {
    goBack($counter_number, 'GIS form must be approved and locked before using Assessed & Paid.');
}

$update = $conn->prepare("UPDATE queue_entries SET status = 'paid', workflow_status = 'PAID', assessed_at = IFNULL(assessed_at, NOW()), paid_at = NOW(), released_at = NOW(), table_number = NULL, counter_number = NULL WHERE id = ?");
$update->bind_param("i", $queue_id);

if ($update->execute()) goBack($counter_number, 'Queue assessed and paid successfully: ' . $row['queue_number']);
