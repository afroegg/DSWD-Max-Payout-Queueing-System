...
function renderQueueRows($queue_entries) {
    ob_start();
    ?>
        ...
        <?php foreach ($queue_entries as $entry): ?>
            ...
            <tr class="<?php echo $rowClass; ?>">
                <td class="<?php echo $isPriority ? 'queue-priority' : ''; ?>">
                    ...
                    <?php if ($isPriority): ?><span class="priority-label">PRIO</span><?php endif; ?>
                </td>
                ...
                <td>
                    ...
                    <?php if ($isCalled): ?>
                        <span class="assigned-counter">Counter <?php echo intval($entry['counter_number']); ?></span>
                    <?php endif; ?>
                    <br>
                    <?php if ($isGISLocked): ?>
                        <span class="gis-badge gis-locked">GIS Locked: ₱<?php echo number_format(floatval($entry['approved_cash_amount']), 2); ?></span>
                    <?php else: ?>
                        <span class="gis-badge gis-draft">GIS Not Locked</span>
                    <?php endif; ?>
                </td>
                <td>
                    <div class="action-buttons">
                        ...
                        <form method="POST" action="../api/cancel_queue.php" class="action-form" onsubmit="return confirm('Cancel this queue only? Beneficiary record will stay saved.');">
                            <input type="hidden" name="queue_id" value="<?php echo intval($entry['id']); ?>">
                            <input type="hidden" name="redirect" value="../staff/counter.php">
                            <button type="submit" class="action-btn cancel" title="Cancel Queue"><span class="material-icons">cancel</span>Cancel</button>
                        </form>
                    </div>
                </td>
            </tr>
...
$queue_entries = getQueueEntries($conn);

if (isset($_GET['ajax']) && $_GET['ajax'] === '1') {
    echo renderQueueRows($queue_entries);
    exit;
}
?>
<!DOCTYPE html>
        ...
        <section class="content">
            ...
            <div class="table-wrap" id="tableWrap">
                <table class="counter-table">
                    ...
                    <tbody id="counterRows"><?php echo renderQueueRows($queue_entries); ?></tbody>
                </table>
            </div>
        </section>
    </main>
...
<script>
    ...
    function saveCounterSelections() {
        document.querySelectorAll('.counter-select').forEach(select => {
            if (!select.disabled) localStorage.setItem('counter_select_' + select.dataset.queueId, select.value);
        });
    }
    ...
    function restoreCounterSelections() {
        document.querySelectorAll('.counter-select').forEach(select => {
            ...
            if (saved && !select.disabled) select.value = saved;
        });
    }
    ...
    async function shadowRefresh() {
        if (isSubmitting || isInteractingWithCounter || document.querySelector('.counter-select:focus')) {
            scheduleRefresh();
            return;
        }

        const oldScrollTop = tableWrap.scrollTop;
        const oldScrollLeft = tableWrap.scrollLeft;
        saveCounterSelections();

        try {
            const response = await fetch('counter.php?ajax=1&ts=' + Date.now(), { cache: 'no-store' });
            const html = await response.text();
            counterRows.innerHTML = html;
            restoreCounterSelections();
            bindCounterEvents();
            tableWrap.scrollTop = oldScrollTop;
            tableWrap.scrollLeft = oldScrollLeft;
        } catch (error) {
            console.error(error);
        }
    ...
    function scheduleRefresh() {
        ...
        refreshTimer = setTimeout(shadowRefresh, 5000);
    }
