...
function lock(disabled){
  document.querySelectorAll('.assist').forEach(btn=>btn.disabled=disabled);
}
async function generateQueue(program){
  msg('Generating queue...');
  lock(true);
  try{
    const response=await fetch('../api/kiosk_generate_queue.php',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({program_type:program,source:'kiosk'})
    });
    const data=await response.json();
    if(!data.success) throw Error(data.message||'Unable to generate queue');
    lastQueue=data;
    queueNo.textContent=data.queue_number;
    summary.textContent=(data.program_type||program)+' - MIMAROPA';
    setStub(data);
    modal.style.display='flex';
    msg('Queue number ready.');
  }catch(error){
...
function setStub(data){
  stubQueue.textContent=data.queue_number||'-';
  stubCode.textContent=data.beneficiary_code||'-';
  stubProgram.textContent=data.program_type||'-';
  stubDate.textContent=data.date_time||new Date().toLocaleString();
}
function printStub(){
  if(lastQueue) setStub(lastQueue);
  setTimeout(()=>print(),80);
}
